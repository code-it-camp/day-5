...
def test(func, inputs, q_string, print_q_string):
	...
	try:
		tests = []
		for inp in inputs:
			# each call is a tuple of inputs for the expected
			# returned function including the first call on the
			# original function
			ret = func
			for call in inp[0]:
				ret = ret(*call)
			
			# final val of ret is usually not expected to be a func.
			tests.append((ret, inp[1], inp[0]))
